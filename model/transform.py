...
def _resize_image_and_masks(image: Tensor, self_min_size: float, self_max_size: float,
                            ...
                            fixed_size: Optional[Tuple[int, int]] = None,segm_names: Optional[List[str]]=None
    ...
    # For multiple segmentation tasks loop to resize each one
    if segm_names:
...
class GeneralizedRCNNTransform(nn.Module):
    ...
    def __init__(self, min_size, max_size, image_mean, image_std, size_divisible=32, fixed_size=None,segm_names=None):
    ...
    def resize(self,
        ...
        image, target = _resize_image_and_masks(image, size, float(self.max_size), target, self.fixed_size,segm_names=self.segm_names)
