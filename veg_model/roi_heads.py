...
def get_class_targets(gt_class,matched_idxs):
    gt_class = gt_class.to(matched_idxs)
    gt_class=gt_class[matched_idxs]
    return gt_class
    #pass
...
def headclass_loss(class_logits, gt_class, gt_labels, mask_matched_idxs):
    ...
    Return:
        class_loss (Tensor): scalar tensor containing the loss
    """
...
def headclass_inference(x, labels):
    ...
    probability (which are of fixed size and directly output
    by the CNN) and return the class in the class field of the BoxList.

    Args:
        x (Tensor): the class logits
        labels (list[BoxList]): bounding boxes that are used as
            reference, one for ech image
...
class RoIHeads(nn.Module):
    ...
    def has_headclass(self):
        if self.class_roi_pool is None:
            return False
    ...
    def forward(self,
        ...
        if self.has_headclass():
            ...
            if self.training:
                ...
                for img_id in range(num_images):
                    pos = torch.where(labels[img_id] > 0)[0]
                    class_proposals.append(proposals[img_id][pos])
                    pos_matched_idxs.append(matched_idxs[img_id][pos])
            else:
                pos_matched_idxs = None

            if self.class_roi_pool is not None:
                ...
                raise Exception("Expected class_roi_pool to be not None")

            loss_class = {}
            if self.training:
                assert targets is not None
                assert pos_matched_idxs is not None
                assert class_logits is not None
                # For multi class level classification loop
                loss_class={}
                for i in range(len(self.class_roi_pool)):
